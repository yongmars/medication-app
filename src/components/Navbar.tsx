  ...
  const getTabClass = (path: string) => {
    ...
    return `flex h-full w-full flex-col items-center justify-center transition-colors ${
      active
        ...
        : "text-slate-400 hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-slate-700/30"
    }`;
  };

  return (
    <nav className="absolute bottom-0 z-50 flex h-16 w-full animate-slide-in-fast items-center justify-around border-t border-blue-100 bg-white dark:border-slate-700 dark:bg-slate-800">
      <Link href="/" className={getTabClass("/")}>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        <span className="mt-1 text-[10px] font-medium">ホーム</span>
      </Link>
      <Link href="/calendar" className={getTabClass("/calendar")}>
        ...
        <span className="mt-1 text-[10px] font-medium">カレンダー</span>
      </Link>
      <Link href="/settings" className={getTabClass("/settings")}>
        ...
        <span className="mt-1 text-[10px] font-medium">設定</span>
      </Link>
    </nav>
  );
}
